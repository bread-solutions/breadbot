...
export const command: ICommand = {
    ...
    run: async(client, message, args, con) => {
        ...
        (message.channel as TextChannel).permissionOverwrites.edit(member.id, { VIEW_CHANNEL: false, SEND_MESSAGES: false }).then(() => {
            message.channel.send({ content: `I removed ${member} from ticket` })
        });
    }
}
